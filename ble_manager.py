...
class BLEManager:
    ...
    def __init__(self, mac, token, state, config):
        ...
        # 端口状态持久化：断联时保存 PIID 16 值，重连后恢复；首次启动（开机自启/每日重启）关闭所有端口
        self._saved_port_ctl: Optional[int] = None  # 断联前保存的端口控制值
        self._has_connected_before: bool = False     # 是否曾经成功连接过（区分首次启动与断联重连）
    ...
    async def _connect(self):
        ...
        # === 端口状态恢复/初始化 ===
        # 首次启动（开机自启/每日重启）：关闭所有端口
        # 断联重连：恢复断联前的端口状态，保证连续性
        if self._has_connected_before and self._saved_port_ctl is not None:
            current_ctl = self.state.settings.get("16", 0x0F)
            if current_ctl != self._saved_port_ctl:
                _LOGGER.info("Reconnect: restoring port state 0x%02X -> 0x%02X", current_ctl, self._saved_port_ctl)
                await self.ctrl.send_miot_command(2, 16, value=self._saved_port_ctl)
                await self.state.update_settings({"16": self._saved_port_ctl})
            else:
                _LOGGER.info("Reconnect: port state already matches saved 0x%02X", current_ctl)
        else:
            _LOGGER.info("First connect (boot/daily restart): turning off all ports")
            await self.ctrl.send_miot_command(2, 16, value=0x00)
            await self.state.update_settings({"16": 0x00})
            for piid in range(1, 5):
                await self.state.update_port(piid, PORT_DEFAULT)
        self._has_connected_before = True
        self._saved_port_ctl = None
    ...
    async def _disconnect(self):
        if self.ctrl:
            ...
            # 保存断联前的端口状态（PIID 16），用于重连后恢复
            self._saved_port_ctl = self.state.settings.get("16", 0x0F)
            _LOGGER.info("Saved port state before disconnect: 0x%02X", self._saved_port_ctl)
